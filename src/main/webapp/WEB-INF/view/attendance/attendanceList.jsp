      ...
      <div>
        <table>
          ...
          <c:forEach var="attendanceDTO" items="${ attendanceList }">
            <tr class="table-content" onclick="location.href='/attendance/attendanceDetail.do?email=${ attendanceDTO.email }&page=1'">
              <td>${ index }</td>
    ...
    <div class="page-wrapper">
      <c:choose>
        ...
        <c:otherwise>
          ...
          <div>
            <c:if test="${ startNum >= 6 }">
              <a href="?keyword=${ keyword }&page=1"><img src="../../../images/angle-double-small-left.svg" alt="처음 페이지로"></a>
            </c:if>
          </div>
          <%-- 이전 블록 --%>
          <div>
            <c:if test="${ startNum - 1 > 0 }">
              <a href="?keyword=${ keyword }&page=${ startNum - 1 }"><img src="../../../images/angle-small-left.svg" alt="이전"></a>
            </c:if>
          </div>
          <%-- 블록 번호 --%>
          <ul class="page-number">
          ...
          <div>
            <c:if test="${ startNum + 4 < endNum }">
              <a href="?keyword=${ keyword }&page=${ startNum + 5 }"><img src="../../../images/angle-small-right.svg" alt="다음"></a>
            </c:if>
          </div>
          <%-- 마지막 블록 --%>
          <div>
            <c:if test="${ startNum + 4 < endNum }">
              <a href="?keyword=${ keyword }&page=${ endNum }"><img src="../../../images/angle-double-small-right.svg" alt=" 마지막 페이지로"></a>
            </c:if>
          </div>
        </c:otherwise>
      </c:choose>
